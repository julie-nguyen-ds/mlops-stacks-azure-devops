...
git add README.md docs .gitignore mlops_databricks_azure_devops/resources/README.md
...
- **`mlops-databricks-azure-devops-tests-ci.yml`**:<br>
  - **[CI]** Performs unit and integration tests<br>
    - Triggered on PR to main
- **`mlops-databricks-azure-devops-bundle-cicd.yml`**:<br>
  - **[CI]** Performs validation of Databricks resources defined under `mlops_databricks_azure_devops/resources`<br>
...
1. Create a new variable group called `mlops-databricks-azure-devops variable group` defining the following secret variables, for more details [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/library/variable-groups?view=azure-devops&tabs=classic#create-a-variable-group):
...
1. Update pipeline YAML files to use service connections rather than pipeline variables:
   - First, remove any lines where the environment variables are set in tasks in `mlops-databricks-azure-devops-tests-ci.yml` or `mlops-databricks-azure-devops-bundle-cicd.yml` files. Specifically, any lines where the following env vars are used: `PROD_AZURE_SP_TENANT_ID`, `PROD_AZURE_SP_APPLICATION_ID`, `PROD_AZURE_SP_CLIENT_SECRET`, `STAGING_AZURE_SP_TENANT_ID`, `STAGING_AZURE_SP_APPLICATION_ID`, `STAGING_AZURE_SP_CLIENT_SECRET`
...
Follow the instructions in [mlops-databricks-azure-devops/resources/README.md](../mlops_databricks_azure_devops/resources/README.md) to deploy ML resources
and production jobs.
...
[ML resource config guide](../mlops_databricks_azure_devops/resources/README.md)  to propose, test, and deploy
